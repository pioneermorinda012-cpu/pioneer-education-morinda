...
const VOUCHER_CODES = [
  ...
  "NARINDER01","NARINDER02","NARINDER03","NARINDER04","NARINDER05",
...
const REMINDERS = [
  ...
  "💡 Did you know? Learning 10 words a day equals 3,650 words a year!",
...
async function fetchVocab(topic,level){
  const prompt="Generate exactly 20 German vocabulary words for the topic \""+topic+"\" at CEFR level "+level+". Return ONLY a JSON array, no explanation, no markdown, no code blocks. Format: [{\"de\":\"German word\",\"en\":\"English translation\",\"emoji\":\"single relevant emoji\"}]. Make sure words are appropriate for "+level+" level learners.";
  try{
...
async function fetchQuiz(topic,level){
  const prompt="Generate 8 multiple choice quiz questions testing German vocabulary for topic \""+topic+"\" at "+level+" level. Return ONLY a JSON array, no explanation, no markdown. Format: [{\"question\":\"German word\",\"correct\":\"English answer\",\"options\":[\"correct\",\"wrong1\",\"wrong2\",\"wrong3\"]}]. Make options plausible but only one correct.";
  try{
    const res=await fetch("https://api.anthropic.com/v1/messages",{method:"POST",headers:{"Content-Type":"application/json"},body:JSON.stringify({model:"claude-sonnet-4-20250514",max_tokens:1000,messages:[{role:"user",content:prompt}]})});
    const data=await res.json();
...
async function fetchExercises(topic,level){
  const prompt="Generate 6 fill-in-the-blank German grammar exercises for topic \""+topic+"\" at "+level+" level. Return ONLY a JSON array, no explanation, no markdown. Format: [{\"sentence\":\"German sentence with ___ for the blank\",\"answer\":\"the missing word\",\"hint\":\"English meaning in brackets\"}]";
  try{
...
function LoadingCard({text}){
  return(
    <div style={{textAlign:"center",padding:48,background:"#f9f9f9",borderRadius:16,border:"2px dashed #ddd"}}>
      ...
      <div style={{fontWeight:600,fontSize:16,color:"#555"}}>{text}</div>
      <div style={{marginTop:16,fontSize:13,color:"#aaa"}}>Powered by AI · Please wait...</div>
    </div>
  );
}
...
function NotifBanner({msg,onClose}){
  return(
    <div style={{position:"fixed",top:0,left:"50%",transform:"translateX(-50%)",width:"100%",maxWidth:680,background:"#1a1a1a",color:"#fff",padding:"12px 16px",zIndex:999,display:"flex",alignItems:"center",gap:10}}>
...
function UpsellBanner({onDismiss,studentName}){
  return(
    <div style={{background:"linear-gradient(135deg,#1a1a1a,#3a0000)",color:"#fff",borderRadius:14,padding:18,marginBottom:14,border:"2px solid #FFCE00"}}>
      <div style={{fontWeight:700,fontSize:15,marginBottom:6}}>💎 Unlock Full A2 + B1 Access!</div>
      <div style={{fontSize:13,opacity:.85,marginBottom:12}}>Get 60 topics, 1500+ AI-generated words, exercises and games for just {COURSE_PRICE}.</div>
...
export default function App(){
  const[users,setUsers]=useStorage("pem_v4_users",{});
  const[usedVouchers,setUsedVouchers]=useStorage("pem_v4_vouchers",[]);
  const[currentUser,setCurrentUser]=useStorage("pem_v4_current",null);
  ...
  const[selectedTopic,setSelectedTopic]=useState(TOPICS.A1[0]);
  ...
  const[lastVisit,setLastVisit]=useStorage("pem_v4_lastvisit",null);
  ...
  useEffect(()=>{
    ...
    if(hoursSince>8)setNotification(REMINDERS[Math.floor(Math.random()*REMINDERS.length)]);
    setLastVisit(now);
  },[currentUser]); // eslint-disable-line

  useEffect(()=>{
    if(!currentUser||!users[currentUser]||users[currentUser].paid)return;
    const t=setTimeout(()=>setShowUpsell(true),25000);
    return()=>clearTimeout(t);
  },[currentUser]); // eslint-disable-line

  async function loadVocab(topic,level){
    const key=level+"_"+topic;
    if(vocabCache.current[key]){setVocabData(vocabCache.current[key]);return;}
    setVocabLoading(true);setVocabData(null);
    const data=await fetchVocab(topic,level);
    if(data){vocabCache.current[key]=data;setVocabData(data);}
    setVocabLoading(false);
  }

  async function loadQuiz(topic,level){
    setQuizLoading(true);setQuizData(null);
    setQuizState({idx:0,score:0,answered:null,done:false});
    const data=await fetchQuiz(topic,level);
    if(data)setQuizData(data);
    setQuizLoading(false);
  }

  async function loadExercises(topic,level){
    setExLoading(true);setExData(null);
    setFillState({idx:0,input:"",result:null,done:false,score:0});
    const data=await fetchExercises(topic,level);
  ...
  function redeemVoucher(){
    ...
    setVoucherMsg("✅ Full access unlocked! Herzlichen Glückwunsch! 🎉");setShowUpsell(false);
  }
  ...
  function getLevel(xp){
    ...
    return{label:"Beginner A1",color:"#DD0000"};
  }

  function generateCode(){
    const chars="ABCDEFGHJKLMNPQRSTUVWXYZ23456789";
    setGeneratedCode("PEM-"+Array.from({length:8},()=>chars[Math.floor(Math.random()*chars.length)]).join(""));
  }
  ...
  const s={
    ...
    header:{background:"#1a1a1a",color:"#fff",padding:"12px 16px",display:"flex",alignItems:"center",gap:10},
    card:{background:"#fff",borderRadius:12,border:"1px solid #eee",padding:16,marginBottom:12},
    btn:(c)=>({background:c||"#DD0000",color:"#fff",border:"none",borderRadius:8,padding:"10px 20px",cursor:"pointer",fontWeight:600,fontSize:14}),
    ...
    nb:(a)=>({flex:1,padding:"6px 2px",border:"none",background:a?"#fff8e1":"transparent",cursor:"pointer",fontSize:9,color:a?"#DD0000":"#888",display:"flex",flexDirection:"column",alignItems:"center",gap:1}),
    lt:(a)=>({flex:1,padding:"8px",border:"none",borderRadius:8,background:a?"#DD0000":"#f3f3f3",color:a?"#fff":"#555",fontWeight:600,cursor:"pointer",fontSize:13}),
  };
  ...
  if(!currentUser) return(
    <div style={s.wrap}>
      ...
      <div style={s.header}>
        ...
        <div><div style={{fontWeight:700,fontSize:17}}>Pioneer Education Morinda</div><div style={{fontSize:11,opacity:.7}}>Deutsch A1-B1 · 1500+ Words</div></div>
      </div>
      {adminOpen?(
        <div style={{padding:"20px 16px"}}>
          <button onClick={()=>setAdminOpen(false)} style={{background:"none",border:"none",color:"#DD0000",cursor:"pointer",fontWeight:600,marginBottom:14,padding:0}}>Back</button>
          {adminPass!==ADMIN_PASSWORD?(
            ...
            <div>
                ...
                <div key={u.name} style={{...s.card,padding:"10px 14px"}}>
                  <div style={{display:"flex",justifyContent:"space-between",alignItems:"center"}}>
                    <div>
                      ...
                      <div style={{fontSize:12,color:"#888"}}>⭐ {u.xp||0} XP</div>
                    </div>
              ...
              <div style={{...s.card,background:"#fffbea"}}>
                ...
                <div style={{fontSize:12,color:"#666",marginBottom:10}}>Generate a code, copy it, and send to student on WhatsApp. They enter it in the app to unlock full access.</div>
                <button style={{...s.btn(),width:"100%",marginBottom:10}} onClick={generateCode}>Generate New Code</button>
                {generatedCode&&(
                  <div>
                    <div style={{fontWeight:800,fontSize:22,color:"#DD0000",letterSpacing:3,textAlign:"center",padding:12,background:"#fff",borderRadius:10,border:"2px dashed #DD0000",marginBottom:8}}>{generatedCode}</div>
                    <button onClick={()=>{try{navigator.clipboard.writeText(generatedCode);alert("Code copied! Send this to the student on WhatsApp.");}catch(e){alert("Code: "+generatedCode);}}} style={{...s.btn("#333"),width:"100%",fontSize:13}}>Copy Code</button>
                  </div>
                )}
              </div>
            </div>
        ...
        <div style={{padding:"20px 16px"}}>
          ...
          <div style={{display:"flex",marginBottom:14,borderRadius:10,overflow:"hidden",border:"1.5px solid #ddd"}}>
            {["login","register"].map(t=>(
              <button key={t} onClick={()=>{setAuthScreen(t);setAuthError("");}} style={{flex:1,padding:"10px",border:"none",background:authScreen===t?"#DD0000":"#fff",color:authScreen===t?"#fff":"#333",fontWeight:600,cursor:"pointer",fontSize:14}}>
          ...
          {authError&&<div style={{color:"#DD0000",fontSize:13,marginBottom:10}}>{authError}</div>}
          <button style={{...s.btn(),width:"100%",padding:13}} onClick={authScreen==="login"?login:register}>
            {authScreen==="login"?"Login":"Create Account"}
          </button>
          <div style={{...s.card,background:"linear-gradient(135deg,#1a1a1a,#2d0000)",border:"2px solid #FFCE00",color:"#fff",textAlign:"center",marginTop:14}}>
            <div style={{fontSize:26,marginBottom:6}}>💎</div>
            <div style={{fontWeight:700,fontSize:16,marginBottom:4}}>Upgrade to Premium</div>
            <div style={{fontSize:22,fontWeight:800,color:"#FFCE00",marginBottom:8}}>{COURSE_PRICE}</div>
            <div style={{fontSize:13,opacity:.8,marginBottom:12,textAlign:"left"}}>
              <div>✅ 60 topics across A1, A2 and B1</div>
              <div>✅ 1500+ AI-generated vocabulary words</div>
              <div>✅ Audio pronunciation for every word</div>
              <div>✅ Unlimited quizzes and exercises</div>
              <div>✅ Progress tracking and XP system</div>
            </div>
            ...
            <div style={{fontSize:11,opacity:.6}}>WhatsApp teacher - Pay - Get voucher - Enter in app - Full access!</div>
          </div>
  ...
  const currentTopics=TOPICS[selectedLevel]||[];

  return(
    <div style={s.wrap}>
      {notification&&<NotifBanner msg={notification} onClose={()=>setNotification(null)}/>}
      <div style={{height:notification?48:0}}/>
      <div style={s.flagBar}/>
      <div style={s.header}>
        <span style={{fontSize:20}}>🇩🇪</span>
        <div><div style={{fontWeight:700,fontSize:15}}>Pioneer Education Morinda</div><div style={{fontSize:10,opacity:.7}}>Deutsch A1-B1 · AI Powered</div></div>
        <div style={{marginLeft:"auto",display:"flex",alignItems:"center",gap:6}}>
          {isPaid&&<div style={{background:"#FFCE00",color:"#1a1a1a",borderRadius:20,padding:"2px 8px",fontSize:10,fontWeight:700}}>💎</div>}
      ...
      <div style={{padding:"12px 14px 0"}}>
          ...
          <div>
            <div style={{...s.card,background:"linear-gradient(135deg,#1a1a1a,#333)",color:"#fff",border:"none"}}>
              <div style={{display:"flex",alignItems:"center",gap:12}}>
                <div style={{width:46,height:46,borderRadius:"50%",background:"#FFCE00",display:"flex",alignItems:"center",justifyContent:"center",fontWeight:700,fontSize:20,color:"#1a1a1a"}}>{user.name[0].toUpperCase()}</div>
                <div>
                  <div style={{fontWeight:700,fontSize:16}}>Hallo, {user.name}! 👋</div>
                  <div style={{fontSize:11,opacity:.7}}>📱 {user.phone}</div>
                </div>
            ...
            {!isPaid&&(
              <div style={{...s.card,background:"linear-gradient(135deg,#fef9e7,#fffbea)",border:"2px solid #FFCE00"}}>
                <div style={{fontWeight:700,fontSize:15,marginBottom:4}}>🎟️ Have a voucher code?</div>
                <div style={{display:"flex",gap:8,marginBottom:8}}>
                  <input value={voucherInput} onChange={e=>setVoucherInput(e.target.value)} placeholder="Enter voucher code" style={{...s.inp,marginBottom:0,flex:1}}/>
                  <button style={s.btn("#1a1a1a")} onClick={redeemVoucher}>Apply</button>
                </div>
                {voucherMsg&&<div style={{fontSize:13,fontWeight:600,color:voucherMsg.startsWith("✅")?"#16a34a":"#DD0000",marginBottom:8}}>{voucherMsg}</div>}
                <button onClick={()=>openWhatsApp(user.name)} style={{background:"#25D366",color:"#fff",border:"none",borderRadius:8,padding:10,fontWeight:700,cursor:"pointer",fontSize:14,width:"100%"}}>
                  💬 Contact Teacher on WhatsApp - {COURSE_PRICE}
                </button>
              </div>
            )}
            {showUpsell&&!isPaid&&<UpsellBanner onDismiss={()=>setShowUpsell(false)} studentName={user.name}/>}
        ...
        {["lessons","flashcards","quiz","exercises"].includes(screen)&&(
          <div>
            <div style={{fontWeight:700,fontSize:18,marginBottom:8}}>
              {screen==="lessons"?"📚 Lessons":screen==="flashcards"?"🃏 Flashcards":screen==="quiz"?"🧠 Quiz":"✏️ Exercises"}
            </div>
            <div style={{display:"flex",gap:6,marginBottom:10}}>
              {["A1","A2","B1"].map(l=>(
                <button key={l} style={s.lt(selectedLevel===l)} onClick={()=>{if(!locked(l)){setSelectedLevel(l);setVocabData(null);setQuizData(null);setExData(null);setSelectedTopic(TOPICS[l][0]);}else setShowUpsell(true);}}>
                  {l}{locked(l)?" 🔒":""}
                </button>
              ))}
            </div>
            {locked(selectedLevel)?(
              <UpsellBanner onDismiss={()=>setSelectedLevel("A1")} studentName={user.name}/>
            ):(
              <div>
                <select value={selectedTopic} onChange={e=>{setSelectedTopic(e.target.value);setVocabData(null);setQuizData(null);setExData(null);}} style={{width:"100%",padding:"10px 14px",fontSize:14,borderRadius:8,border:"1.5px solid #ddd",outline:"none",background:"#fff",marginBottom:12}}>
                  ...
                  <div>
                    {!vocabData&&!vocabLoading&&(
                      <div style={{textAlign:"center",padding:24}}>
                        ...
                        <div style={{fontWeight:600,fontSize:16,marginBottom:4}}>{selectedTopic}</div>
                        <div style={{color:"#888",fontSize:13,marginBottom:16}}>Level {selectedLevel} · 20 vocabulary words with audio</div>
                        <button style={{...s.btn(),padding:"12px 32px"}} onClick={()=>loadVocab(selectedTopic,selectedLevel)}>Load Vocabulary 🤖</button>
                      </div>
                    )}
                    {vocabLoading&&<LoadingCard text={"Generating "+selectedTopic+" vocabulary..."}/>}
                    {vocabData&&(
                      <div>
                        <div style={{fontSize:13,color:"#888",marginBottom:10}}>{vocabData.length} words · {selectedLevel} · {selectedTopic}</div>
                        {vocabData.map((v,i)=>(
                        ...
                        <button style={{...s.btn("#666"),width:"100%",marginTop:4}} onClick={()=>{vocabCache.current[selectedLevel+"_"+selectedTopic]=null;loadVocab(selectedTopic,selectedLevel);}}>🔄 Regenerate</button>
                      </div>
                    )}
                  </div>
                )}
                  ...
                  <div>
                    {!vocabData&&!vocabLoading&&(
                      <div style={{textAlign:"center",padding:24}}>
                        <div style={{fontSize:42,marginBottom:8}}>🃏</div>
                        <div style={{fontWeight:600,fontSize:16,marginBottom:4}}>{selectedTopic}</div>
                        <button style={{...s.btn(),padding:"12px 32px"}} onClick={()=>loadVocab(selectedTopic,selectedLevel)}>Load Cards 🤖</button>
                      </div>
                    )}
                    {vocabLoading&&<LoadingCard text="Generating flashcards..."/>}
                    {vocabData&&vocabData.length>0&&(()=>{
                      ...
                      return(
                        <div>
                          ...
                          <div onClick={()=>setFlipped(f=>!f)} style={{background:flipped?"#fff8e1":"#e8f4fd",borderRadius:16,border:"2px solid "+(flipped?"#FFCE00":"#90caf9"),padding:24,textAlign:"center",cursor:"pointer",minHeight:180,display:"flex",flexDirection:"column",alignItems:"center",justifyContent:"center",gap:8,marginBottom:12}}>
                          ...
                          <div style={{display:"flex",gap:8}}>
                            ...
                            <button style={{...s.btn("#DD0000"),flex:1}} onClick={()=>{setCardIdx(i=>i+1);setFlipped(false);}}>Again</button>
                          </div>
                        </div>
                      );
                    })()}
                ...
                {screen==="quiz"&&(
                  <div>
                    {!quizData&&!quizLoading&&(
                      <div style={{textAlign:"center",padding:24}}>
                        <div style={{fontSize:42,marginBottom:8}}>🧠</div>
                        <div style={{fontWeight:600,fontSize:16,marginBottom:4}}>{selectedTopic} Quiz</div>
                        <div style={{color:"#888",fontSize:13,marginBottom:16}}>8 questions · Level {selectedLevel}</div>
                        <button style={{...s.btn(),padding:"12px 32px"}} onClick={()=>loadQuiz(selectedTopic,selectedLevel)}>Start Quiz 🤖</button>
                      </div>
                    )}
                    {quizLoading&&<LoadingCard text="Generating quiz questions..."/>}
                    {quizData&&(
                      quizState.done?(
                        <div style={{...s.card,textAlign:"center",padding:32}}>
                          <div style={{fontSize:52}}>{quizState.score>=6?"🏆":quizState.score>=4?"🎉":"💪"}</div>
                          <div style={{fontSize:26,fontWeight:700}}>{quizState.score}/{quizData.length}</div>
                          <div style={{color:"#DD0000",fontWeight:600,marginTop:6}}>+{quizState.score*10} XP!</div>
                          <div style={{display:"flex",gap:8,marginTop:14,justifyContent:"center"}}>
                            <button style={s.btn()} onClick={()=>{updateScore("quiz_"+selectedLevel+"_"+selectedTopic,quizState.score,quizData.length);setQuizState({idx:0,score:0,answered:null,done:false});}}>Try Again</button>
                            <button style={s.btn("#555")} onClick={()=>loadQuiz(selectedTopic,selectedLevel)}>New Quiz 🤖</button>
                          </div>
                        </div>
                      ):(
                        <div>
                          ...
                          <div style={{background:"#eee",borderRadius:8,height:5,marginBottom:12}}>
                            <div style={{width:((quizState.idx/quizData.length)*100)+"%",height:"100%",background:"#DD0000",borderRadius:8}}/>
                          </div>
                          <div style={{...s.card,textAlign:"center",background:"#e8f4fd",border:"2px solid #90caf9",marginBottom:12,padding:20}}>
                            ...
                            <div style={{fontSize:28,fontWeight:800}}>{quizData[quizState.idx].question}</div>
                  ...
                  <div>
                    {!exData&&!exLoading&&(
                      <div style={{textAlign:"center",padding:24}}>
                        ...
                        <div style={{fontWeight:600,fontSize:16,marginBottom:4}}>{selectedTopic} Exercises</div>
                        <div style={{color:"#888",fontSize:13,marginBottom:16}}>6 fill-in-the-blank exercises · Level {selectedLevel}</div>
                        <button style={{...s.btn(),padding:"12px 32px"}} onClick={()=>loadExercises(selectedTopic,selectedLevel)}>Generate Exercises 🤖</button>
                      </div>
                    )}
                    {exLoading&&<LoadingCard text="Generating exercises..."/>}
                    {exData&&(
                      fillState.done?(
                        <div style={{...s.card,textAlign:"center",padding:32}}>
                          <div style={{fontSize:48}}>🎉</div>
                          <div style={{fontSize:24,fontWeight:700}}>{fillState.score}/{exData.length}</div>
                          <div style={{color:"#DD0000",fontWeight:600,marginTop:6}}>+{fillState.score*10} XP!</div>
                          <div style={{display:"flex",gap:8,marginTop:14,justifyContent:"center"}}>
                            <button style={s.btn()} onClick={()=>{updateScore("ex_"+selectedLevel+"_"+selectedTopic,fillState.score,exData.length);setFillState({idx:0,input:"",result:null,done:false,score:0});}}>Try Again</button>
                            <button style={s.btn("#555")} onClick={()=>loadExercises(selectedTopic,selectedLevel)}>New Exercises 🤖</button>
                          </div>
                        </div>
                      ):(
                        <div>
        ...
        {screen==="progress"&&(
          <div>
            ...
            <div style={{...s.card,background:"linear-gradient(135deg,#1a1a1a,#333)",color:"#fff",border:"none",textAlign:"center",padding:20}}>
              <div style={{width:54,height:54,borderRadius:"50%",background:"#FFCE00",display:"flex",alignItems:"center",justifyContent:"center",fontWeight:700,fontSize:22,color:"#1a1a1a",margin:"0 auto"}}>{user.name[0].toUpperCase()}</div>
            ...
            {!isPaid&&<UpsellBanner onDismiss={()=>{}} studentName={user.name}/>}
            ...
            {Object.keys(user.scores||{}).length===0?(
              ...
              <div key={key} style={{...s.card,padding:"10px 14px"}}>
                <div style={{display:"flex",justifyContent:"space-between",marginBottom:4}}>
                  ...
                  <span style={{fontWeight:700,color:val>=70?"#22c55e":val>=50?"#d97706":"#DD0000"}}>{val}%</span>
                </div>
                <div style={{background:"#eee",borderRadius:8,height:7,overflow:"hidden"}}>
                  <div style={{width:val+"%",height:"100%",background:val>=70?"#22c55e":val>=50?"#d97706":"#DD0000",borderRadius:8}}/>
                </div>
              </div>
            ))}
          </div>
